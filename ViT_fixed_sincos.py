...
#Create a CustomVisionTRansformer

class CustomVisionTransformer(VisionTransformer):
    def __init__(self, *args, **kwargs):
        super(CustomVisionTransformer, self).__init__(*args, **kwargs)

        self.register_buffer('fixed_pos_embed', self.create_fixed_position_embeddings(self.embed_dim, self.patch_embed.num_patches + self.num_prefix_tokens), persistent=False)

    def create_fixed_position_embeddings(self, embed_dim, num_positions):
        """Create fixed position embeddings."""
        position_ids = torch.arange(0, num_positions).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, embed_dim, 2) * -(math.log(10000.0) / embed_dim))
        
        pos_embed = torch.zeros((num_positions, embed_dim))
        pos_embed[:, 0::2] = torch.sin(position_ids * div_term)
        pos_embed[:, 1::2] = torch.cos(position_ids * div_term)
